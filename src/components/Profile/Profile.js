...
useEffect(() => 
reset({name: currentUser.name, email: currentUser.email}), [currentUser]);
    ...
    <section className="profile" aria-label="Профиль пользователя">
      ...
      <form className="profile__form"  onSubmit={handleSubmit}  noValidate >
        ...
        <div className="profile__input-area profile__input-area_name">
          ...
          <input type="text" 
          className= {`profile__input ${errors.name ? "profile__input_color" : ""}`}
          value={values.name || ""} 
          name="name"
          minLength='2'
          maxLength='30'
        ...
        <div className="profile__input-area">
          ...
          <input type="email" 
          className={`profile__input ${errors.email ? "profile__input_color" : ""}`}
          value={values.email || ""} 
          name="email"
          pattern='^.+@.+\..+$'
          onChange={handleChange}/>
        ...
        <button 
        className={`profile__button  ${!isValid ? "profile__button_disabled": "profile__button_colored"}`} 
        type="submit"
